...
def get_features(data_path: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    mid_price = (np.array(data_file['OB/Ask']).min(axis=1) +
           np.array(data_file['OB/Bid']).max(axis=1)
          ) / 2

    mid_price_weighted = (np.array(data_file['OB/Ask'])[:, 0]*np.array(data_file['OB/AskV'])[:, 0] + \
    ...
    for w in window_sizes:
        ...
        relative_pred_by_trends = (np.hstack([first_w_preds, preds]) - mid_price)/mid_price
        relative_preds_by_trends.append(relative_pred_by_trends)
        slopes_features.append(np.hstack([np.zeros(w - 1), slope]))
        
    volume_spread = (data_file['OB/AskV'][:, 0] - data_file['OB/BidV'][:, 0])/( data_file['OB/AskV'][:, 0] + data_file['OB/BidV'][:, 0])
    normalized_price_spreads = (data_file['OB/Ask'][:, :10] - data_file['OB/Bid'][:, :10])/mid_price.reshape(-1, 1)
    ...
    features = [   askV,
        ...
        [relative_pred_by_trends for relative_pred_by_trends in relative_preds_by_trends] + \
